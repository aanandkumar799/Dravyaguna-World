...
export default function Viva() {
  ...
  const next=()=>{logPlantActivity(q.plantId, "viva"); const updated=recordAttempt(progress,{mode:"viva",itemId:q.id,plantId:q.plantId,outcome:revealed?"revealed":"skipped"});setProgress(updated);window.localStorage.setItem(PROGRESS_STORAGE_KEY,JSON.stringify(updated));setI(i+1);setRevealed(false);};
