...
export default function Flashcards() {
  ...
  const next=()=>{logPlantActivity(card.plantId, "flashcard"); const updated=recordAttempt(progress,{mode:"flashcard",itemId:card.id,plantId:card.plantId,outcome:flip?"revealed":"skipped"});setProgress(updated);window.localStorage.setItem(PROGRESS_STORAGE_KEY,JSON.stringify(updated));setI(i+1);setFlip(false);};
