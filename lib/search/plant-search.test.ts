...
describe("plant search",()=>{it("matches botanical names",()=>expect(searchPlants(plants,"Ocimum").length).toBe(1));it("matches Sanskrit names",()=>expect(searchPlants(plants,"Tulasi")[0]?.plant.id).toBe("plant-tulasi"));});